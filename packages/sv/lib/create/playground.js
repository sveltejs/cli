/** @import { AstTypes } from '../core.js' */
import fs from 'node:fs';
import path from 'node:path';
import { walk } from 'zimmerframe';
import { commonFilePaths } from '../cli/add/utils.js';

import { isVersionUnsupportedBelow, js, parse, svelte } from '../core.js';
import { getSharedFiles } from './utils.js';
 ...
 * @returns {boolean}
 */
export function validatePlaygroundUrl(link) {
	try {
...
/**
 * @param {string} link
 * @returns {PlaygroundURL}
 */
export function parsePlaygroundUrl(link) {
	const url = new URL(link);
...
/**
 * @param {PlaygroundURL} params
 * @returns {Promise<PlaygroundData>}
 */
export async function downloadPlaygroundData({ playgroundId, hash, svelteVersion }) {
	let data = [];
	...
	return {
		name: data.name,
		files: files.map(
			/** @param {{ name: string; type: string; contents: string; source: string }} file */
			(file) => {
				return {
...
/**
 * @param {PlaygroundData['files']} files
 * @returns {Map<string, string>} A Map of packages with it's name as the key, and it's version as the value.
 */
export function detectPlaygroundDependencies(files) {
	...
	for (const file of files) {
		/** @type {AstTypes.Program | undefined} */
		let ast;
		...
		const imports = ast.body
			.filter(
				/** @returns {node is AstTypes.ImportDeclaration} */ (node) =>
					node.type === 'ImportDeclaration'
			)
			.map((node) => /** @type {string} */ (node.source.value))
...
function extractPackageInfo(importPath) {
	...
	if (importPath.startsWith('@')) {
		const [org, pkg] = importPath.split('/', 3);
		pkgName = `${org}/${pkg}`;
	}
...
/**
 * @param {string} url
 * @param {PlaygroundData} playground
 * @param {string} cwd
 * @param {boolean} installDependencies
 * @returns {void}
 */
export function setupPlaygroundProject(url, playground, cwd, installDependencies) {
	...
	// add playground shared files
	{
		...
		for (const file of playgroundFiles) {
			...
			if (file.name === 'src/lib/PlaygroundLayout.svelte') {
				...
				walk(/** @type {AstTypes.Node} */ (ast.instance.content), null, {
					Literal(node) {
