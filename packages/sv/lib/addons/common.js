...
import { js, parse, svelte } from '../core.js';
...
export function addEslintConfigPrettier(content) {
	...
	for (const specifier of sveltePluginImport?.specifiers ?? []) {
		if (specifier.type === 'ImportDefaultSpecifier' && specifier.local?.name) {
			svelteImportName = /** @type {string} */ (specifier.local.name);
		}
	}
 ...
 * @param {string} path
 * @param {'ts' | 'js'} language
 * @returns {string}
 */
export function addToDemoPage(existingContent, path, language) {
	const { ast, generateCode } = parse.svelte(existingContent);

	for (const node of ast.fragment.nodes) {
		if (node.type === 'RegularElement') {
			const hrefAttribute = /** @type {SvelteAst.Attribute | undefined} */ (
				node.attributes.find((x) => x.type === 'Attribute' && x.name === 'href')
			);
