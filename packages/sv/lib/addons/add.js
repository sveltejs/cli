/** @import { Addon, AddonSetupResult, OptionValues, PackageManager, Question, SvApi, Workspace } from '../core.js' */
...
import { fileExists, installPackages, readFile, writeFile } from '../cli/add/utils.js';
import { createWorkspace } from '../cli/add/workspace.js';
import { TESTING } from '../cli/utils/env.js';
...
/**
 * @typedef {Record<string, Addon<any>>} AddonMap
 */

/**
 * @template {AddonMap} Addons
 * @typedef {{ [K in keyof Addons]: Partial<OptionValues<Addons[K]['options']>> }} OptionMap
 */

/**
 * @template {AddonMap} Addons
 * @param {InstallOptions<Addons>} options
 ...
 * @returns {Promise<{ filesToFormat: string[]; pnpmBuildDependencies: string[]; status: Record<string, string[] | 'success'> }>}
 */
...
async function runAddon({ addon, multiple, workspace, workspaceOptions }) {
	...
	const sv = {
		...
		execute: async (commandArgs, stdio) => {
			const result = resolveCommand(workspace.packageManager, 'execute', commandArgs);
			if (!result) throw new Error('Failed to resolve execute command');
