...
/** @import { AddonMap } from '../../add.js' */
/** @import { AddonTestCase, Fixtures, SetupTestOptions } from '../../../testing.js' */
...
import { createProject, addPnpmBuildDependencies, prepareServer } from '../../../testing.js';
 ...
 * @param {Addons} addons
 * @param {SetupTestOptions<Addons>} [options]
 * @returns {{ test: import('vitest').TestAPI<Fixtures>, testCases: Array<AddonTestCase<Addons>>, prepareServer: typeof prepareServer }}
 */
export function setupTest(addons, options) {
	/** @type {import('vitest').TestAPI<Fixtures>} */
	const test = /** @type {any} */ (vitestTest.extend({}));
	...
	/** @type {Array<AddonTestCase<Addons>>} */
	const testCases = [];
	...
	beforeAll(async ({ name }) => {
		testName = /** @type {string} */ (path.dirname(name).split('/').at(-1));
