...
import { AGENTS, COMMANDS, constructCommand, detect } from 'package-manager-detector';
import { exec } from 'tinyexec';

import { color } from '../../cli/add/utils.js';
import { isVersionUnsupportedBelow, parse } from '../../core.js';

/** @type {AgentName[]} */
export const AGENT_NAMES = /** @type {AgentName[]} */ (
	AGENTS.filter((agent) => !agent.includes('@'))
);

/** @type {Array<{ value: AgentName | undefined; label: AgentName | 'None' }>} */
const agentOptions = AGENT_NAMES.map((pm) => ({ value: pm, label: pm }));
...
/**
 * @param {string} cwd
 * @returns {Promise<AgentName | undefined>}
 */
export async function packageManagerPrompt(cwd) {
 ...
 */
export async function installDependencies(agent, cwd) {
	...
	try {
		const result = constructCommand(COMMANDS[agent].install, []);
		if (!result) throw new Error('Failed to construct install command');
...
export function getUserAgent() {
	...
	const name = /** @type {AgentName} */ (pmSpec.substring(0, separatorPos));
	return AGENTS.includes(name) ? name : undefined;
}

/**
 * @param {string} cwd
 * @param {AgentName | null | undefined} packageManager
 * @param {string[]} allowedPackages
 * @returns {Promise<void>}
 */
export async function addPnpmBuildDependencies(cwd, packageManager, allowedPackages) {
	...
	/** @type {'package.json' | 'pnpm-workspace.yaml'} */
	...
	if (confIn === 'pnpm-workspace.yaml') {
		...
		const items = onlyBuiltDependencies?.items ?? [];
