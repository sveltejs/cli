...
const { test, prepareServer, addonTestCases } = setupTest(
	{ tailwindcss },
	{
		kinds: [
...
test.concurrent.for(addonTestCases)(
	...
	async (addonTestCase, { page, ...ctx }) => {
		...
		// ...add test files
		addFixture(cwd, addonTestCase.variant);

		const { close } = await prepareServer({ cwd, page });
		// kill server process when we're done
		ctx.onTestFinished(async () => await close());

		if (addonTestCase.kind.type === 'none') {
		...
		} else if (addonTestCase.kind.type === 'typography') {
