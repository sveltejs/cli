...
export default defineAdder({
	...
	files: [
		...
		{
			name: ({ kit, typescript }) =>
				`${kit!.routesDirectory}/lucia-demo/login/+page.server.${typescript ? 'ts' : 'js'}`,
			condition: ({ options }) => options.demo,
			content({ content, typescript, kit }) {
				if (content) {
					const filePath = `${kit!.routesDirectory}/lucia-demo/login/+page.server.${typescript ? 'ts' : 'js'}`;
				...
				return dedent`
					...
					export const load${ts(': PageServerLoad')} = async (event) => {
						if (event.locals.user) {
							return redirect(302, '/lucia-demo');
						}
						return {};
					};
		...
		{
			name: ({ kit }) => `${kit!.routesDirectory}/lucia-demo/login/+page.svelte`,
			condition: ({ options }) => options.demo,
			content({ content, dependencyVersion, typescript, kit }) {
				if (content) {
					const filePath = `${kit!.routesDirectory}/lucia-demo/login/+page.svelte`;
					log.warn(`Existing ${colors.yellow(filePath)} file. Could not update.`);
					return content;
				}
		...
		{
			name: ({ kit, typescript }) =>
				`${kit!.routesDirectory}/lucia-demo/+page.server.${typescript ? 'ts' : 'js'}`,
			condition: ({ options }) => options.demo,
			content({ content, typescript, kit }) {
				if (content) {
					const filePath = `${kit!.routesDirectory}/lucia-demo/+page.server.${typescript ? 'ts' : 'js'}`;
					log.warn(`Existing ${colors.yellow(filePath)} file. Could not update.`);
					return content;
				}
				...
				return dedent`
					...
					export const load${ts(': PageServerLoad')} = async (event) => {
						if (!event.locals.user) {
							return redirect(302, '/lucia-demo/login');
						}
		...
		{
			name: ({ kit }) => `${kit!.routesDirectory}/lucia-demo/+page.svelte`,
			condition: ({ options }) => options.demo,
			content({ content, dependencyVersion, typescript, kit }) {
				if (content) {
					const filePath = `${kit!.routesDirectory}/lucia-demo/+page.svelte`;
					log.warn(`Existing ${colors.yellow(filePath)} file. Could not update.`);
					return content;
				}
	...
	nextSteps: ({ highlighter, options }) => {
		...
		if (options.demo) {
			steps.push(`Visit ${highlighter.route('/lucia-demo')} route to view the demo`);
		}
