...
Annotate the built options with `AddonOptions<Values>` and export `Values`. The compiler then
checks your questions against the values they must produce, and consumers of your add-on get a
small, readable type to build options with:

```ts
import { type AddonOptions, defineAddonOptions } from 'sv';

// ---cut---
export type MyAddonOptions = { database: 'postgresql' | 'sqlite'; docker: boolean };

export const options: AddonOptions<MyAddonOptions> = defineAddonOptions()
	.add('database', {
		question: 'Which database?',
		type: 'select',
		default: 'sqlite',
...
Official add-ons follow the same pattern; their values are available as `OfficialAddonOptions`:

```ts
import type { OfficialAddonOptions } from 'sv';

// ---cut---
// `add()` takes partial options - unanswered questions fall back to their default
const drizzle: Partial<OfficialAddonOptions['drizzle']> = { database: 'sqlite', docker: false };
```
