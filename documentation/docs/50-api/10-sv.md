...
`runsAfter` makes add-on ordering explicit, while `dependsOn` loosely infers it.

`dependsOn` and `runsAfter` both accepts official and community add-on ids. (`'sveltekit-adapter'`, `'better-auth'`, ...) 
