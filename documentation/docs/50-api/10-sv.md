...
`dependsOn` and `runsAfter` take add-on ids (`'sveltekit-adapter'`, `'better-auth'`, ...), including community ones. A missing `dependsOn` is installed automatically if it's an official add-on, otherwise it has to be part of the same command. `runsAfter` only affects ordering, and `dependsOn` implies it.
